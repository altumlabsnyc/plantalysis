...
export default function LabCurrentOrders() {
  ...
  const { data, error, isLoading } = useLabClaimedOrders(user)
  ...
  const columns = [
    ...
    columnHelper.accessor('id', {
      header: 'Order #',
      cell: (info) => <div>{'...' + info.getValue().slice(-10)}</div>,
    }),
    columnHelper.accessor('analysis_id', {
      header: 'Analysis Uploaded',
      cell: (info) => <div>{info.getValue() == null ? 'No' : 'Yes'}</div>,
    }),
  ...
  return (
    <div className="flex flex-col items-center m-4 gap-4 pb-8">
      <div className="mx-auto">
        <Panel>
          <div className="w-128 h-96 py-2 overflow-y-scroll">
            <>
              {isLoading ? (
                ...
                <>
                  ...
                  <p className="text-xs text-gray-500 mb-2">Claimed Order</p>
