      ...
      .then(({data, error})=>{
        ...
        if (data){
          analysisData = data.map((order)=>{
            ...
            const tests: Array<{test: Test, reqResults: Array<{result: TestResult, requirement: TestRequirement}>}> = order.lab_order_on_test.map((testPreprocessed)=>{
              ...
              const reqResultPairs: Array<{result: TestResult, requirement: TestRequirement}> = testPreprocessed.test.test_requirement.map((reqPreProcessed)=>{
                ...
                if (reqPreProcessed.test_result.length === 0){
                  toast.error('no result associated with a mandatory requirement. Please contact Altum support');
                  throw new Error('every requirement needs to be associated for a result in a given analysis')
                }
